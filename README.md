...
пропсы, как требует задание. На скриншоте пользователь авторизован, поэтому
кнопка «Войти в профиль» не показывается — остаётся только переход к карточкам.
...
| Ветка                        | Назначение                                          |
| ---------------------------- | --------------------------------------------------- |
| `main`                       | продакшен: пуш в неё запускает деплой на Vercel      |
| `dev`                        | интеграционная ветка, куда собираются задачи         |
| `feat/*`, `doc/*`, `build/*` | по ветке на задачу, тип совпадает с типом коммита    |

Задача живёт в своей ветке и приходит в `dev` пул-реквестом с обязательными
проверками. Накопленное в `dev` уходит в `main` отдельным пул-реквестом — после
слияния собирается прод.
